...
Run with ``uv run python -m chess_commentator.dataset.generate [--center]``. Needs the robot and a
...
5. The photo is warped and cut into 64 square cutouts by the existing
   :class:`~chess_commentator.perception.image_processing.Processor`.
...
from chess_commentator.perception.calibration import calibrate
...
class DataGenerationSession:
    ...
    def __init__(
        ...
        # centre is interpolated from the 4 corners. See chess_commentator.perception.calibration.CalibrationUI.
...
if __name__ == "__main__":
    # Usage: uv run python -m chess_commentator.dataset.generate [--center]
    #
